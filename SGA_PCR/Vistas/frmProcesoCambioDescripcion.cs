...
namespace Apolo
{
    public partial class frmProcesoCambioDescripcion : Form
    {
        ...
        public frmProcesoCambioDescripcion()
        {
            InitializeComponent();
            Inicializado();
        }

        public frmProcesoCambioDescripcion(int idUsuario, string nombreUsuario)
        {
            InitializeComponent();
        ...
        private void btnAgregarMemoria_Click(object sender, EventArgs e)
        {
            ...
            if (laptop.IdLC != 0)
            {
                using (frmProcesoEditarMemoria frm = new frmProcesoEditarMemoria(1,"CEAS",1))
                {
                    if (frm.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                    {
                        ...
                        {
                            error = laptopDA.ActualizarLaptopMemoriaSinAfectarCantidad(laptop.IdLC, laptop.Memorias, this.nombreUsuario);
                        }
                        else
                        {
                            error = laptopDA.InsertarLaptopMemoriasPrimeraVezSinAfectarCantidad(laptop.IdLC, laptop.Memorias, this.nombreUsuario);
                        }
