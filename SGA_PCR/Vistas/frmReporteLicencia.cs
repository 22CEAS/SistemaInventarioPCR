...
namespace Apolo
{
    public partial class frmReporteLicencia : Form
    {
        ...
        public frmReporteLicencia()
        {
            InitializeComponent();
            Inicializado();
        }

        public frmReporteLicencia(int idUsuario, string nombreUsuario)
        {
            InitializeComponent();
...
//
        ...
        private void btnExportar_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show("Estas seguro que desea Exportar el reporte", "◄ AVISO | LEASEIN S.A.C. ►", MessageBoxButtons.YesNo, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1) == DialogResult.Yes)
            {
                ...
                try
                {
                    ...
                    string file = "REPORTE LICENCIAS.xlsx";
                    dgvLicencias.ExportToXlsx(file, options);
        ...
        public void ExportarDataGridViewExcel(ref Excel.Worksheet hoja_trabajo, string nombreCabecera)
        {
            ...
            for (int j = 0; j < filas; j++)
            {
                ...
                hoja_trabajo.Cells[fila2, 2] = vista.GetRowCellValue(j, "Marca").ToString();
                hoja_trabajo.Cells[fila2, 3] = vista.GetRowCellValue(j, "Version").ToString();
                hoja_trabajo.Cells[fila2, 4] = vista.GetRowCellValue(j, "Clave").ToString();
                hoja_trabajo.Cells[fila2, 5] = vista.GetRowCellValue(j, "CodigoLC").ToString();
                hoja_trabajo.Cells[fila2, 6] = vista.GetRowCellValue(j, "NombreEstado").ToString();
        ...
        private void montaCabeceras(int fila, ref Excel.Worksheet hoja, string nombreCabecera)
        {
            try
            {
                ...
                hoja.Cells[fila + 2, 2] = "Marca";
                hoja.Cells[fila + 2, 3] = "Versión";
                hoja.Cells[fila + 2, 4] = "Clave";
                hoja.Cells[fila + 2, 5] = "Codigo Laptop";
        ...
        public bool cargarDataTabla()
        {
            try
            {
                ...
                tablaLicencias = reporteDA.ListarLicenciasCompletas();

                dgvLicencias.DataSource = tablaLicencias;
